...
void setup() {
  ...
  // Inicia Bluetooth como cliente (master)
  if (!SerialBT.begin("ESP32_Client", true)) {  // false = master mode
    Serial.println("Error iniciando cliente Bluetooth");
    return;
  }

  // Dirección MAC del servidor al que nos queremos conectar
  uint8_t serverMAC[6] = { 0x1C, 0x69, 0x20, 0xA3, 0x88, 0xBA };

  Serial.println("Intentando conectar al servidor Bluetooth...");

  if (SerialBT.connect(serverMAC)) {
    Serial.println("¡Conexión Bluetooth exitosa!");
    SerialBT.println("Hola desde el cliente ESP32 (por MAC)");
  } else {
    Serial.println("Fallo al conectar por Bluetooth");
  }
